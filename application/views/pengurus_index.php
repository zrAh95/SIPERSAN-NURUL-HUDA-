        ...
        <tbody>
            <?php $no = 1; foreach ($pengurus as $p): ?>
                <tr>
                    <td><?= $no++ ?></td>
                    <td>
                        <?php if (!empty($p->foto) && file_exists(FCPATH . 'uploads/pengurus/' . $p->foto)): ?>
                            <img src="<?= base_url('uploads/pengurus/' . $p->foto) ?>" class="img-thumbnail foto-pengurus" alt="Foto <?= html_escape($p->nama_pengguna) ?>">
                        <?php else: ?>
                            <span class="text-muted">-</span>
                        <?php endif; ?>
                    </td>
                    <td><?= html_escape($p->nama_pengguna) ?></td>
                    <td><?= html_escape($p->no_telp) ?></td>
                    <td>
